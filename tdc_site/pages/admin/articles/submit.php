<?php
	...
	if ($userInfo && $M['permissions']->havePermission($userInfo, 'articles_addarticle'))
	{
		...
		{
			$title = $db->real_escape_string($_POST['article-title']);
			$content = $db->real_escape_string($_POST['article-content']);
			
			// Now for the fun part!
			$query = $db->query("INSERT INTO `articles` (`userid`, `title`, `content`, `timeadded`, `category`) VALUES (" . $userInfo['id'] . ", '" . $title . "', '" . $content . "', " . time() . ", 1);");
			
			if ($query)
			{
				$insertID = $db->insert_id;
				$success = true;
				$message = 'Successfully created the article! You can view it <a href="/index.php">here</a>.';
			}
			else
			{
				$success = false;
